...
admin_log_blueprint = Blueprint("admin_logs", __name__, url_prefix="/logs", template_folder="../../../templates")


@admin_log_blueprint.route("/logs_display", methods=["GET"])
def logs_display():
...
@admin_log_blueprint.route("/logs", methods=["GET"])
def get_logs():
...
@admin_log_blueprint.route("/clear_logs", methods=["POST"])
def clear_logs():
...
@admin_log_blueprint.route("/api/logs-summary", methods=["GET"])
def logs_summary():
    import json
    import re
    ...
    if os.path.exists("blocked_ips.json"):
        with open("blocked_ips.json", "r") as block_file:
            blocked_ips = json.load(block_file)
